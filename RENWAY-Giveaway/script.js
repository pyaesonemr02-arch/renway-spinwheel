 ...
 * Enterprise-grade Canvas Wheel & State Management
 */
...
// Constants
const COLORS = ['#1A1A1A', '#0D0D0D']; // Alternating luxury blacks
const GOLD = '#D4AF37';
...
function renderWheel() {
    ...
    for (let i = 0; i < numSlices; i++) {
        ...
        ctx.fill();
        ctx.lineWidth = 1;
        ctx.strokeStyle = 'rgba(212, 175, 55, 0.3)'; // Subtle gold border
        ctx.stroke();

        // Draw Text (Performance check: limit text rendering if > 200 participants to maintain 60fps)
        if (numSlices <= 200) {
...
function drawText(index, startAngle, sliceAngle, centerX, centerY, radius) {
    ...
    // Truncate long names for wheel aesthetics
...
function spinWheel() {
    ...
    // We pick the winner strictly randomly via crypto (Fair mode)
    ...
    // We want the winning slice to end up at the top (270 degrees or -Math.PI/2)
    const targetAngle = -(Math.PI / 2); 
    const currentBaseAngle = (winningIndex * sliceAngle) + (sliceAngle / 2);
    
    // Add multiple full rotations (e.g., 10 spins = 20 * Math.PI)
    ...
    let startTime = null;
    const duration = 6000; // 6 seconds for luxury dramatic effect
    ...
    function animate(currentTime) {
        ...
        if (progress < 1) {
            ...
            AppState.currentRotation = AppState.currentRotation % (2 * Math.PI); // Normalize
...
function bindEvents() {
    ...
    ui.btnClear.addEventListener('click', () => {
        if(confirm("Clear ALL participants? Winners are saved.")) {
